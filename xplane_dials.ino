...
void loop() {
  // Declair the four sensors and give them name: ADFdial; VORdial; Baro1dial; Baro2dial.
 ...
 FlightSim.update();
  ...
  if (ADFdiff < -180) //increment ADF if shaft passes North anticlock.
  {
    ...
    if (ADF > 5)  //limit ADF to 0 to 360
  ...
  if (VORdiff > 180) //decriment VOR if shaft passes North clockwise.
  {
    ...
    if (VOR < 0)  //limit VOR to 0 to 360
    {
      VOR = 5;
    }
  }
  ...
  //Output to X-Plane only if dataref changes.
  ...
  if (VORdiff >= 2 || VORdiff <= -2)  //output if VORdial is rotated
  {
    VORout = VORobs;
